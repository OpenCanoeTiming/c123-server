...
- For `connect`: requires active C123 server with XmlChangeNotifier and EventState

---

### GET /api/live/status

Get current Live-Mini pusher status.
...
```json
{
  "status": {
    ...
    "eventStatus": "running",
    "channels": {
      "xml": { "enabled": true, "lastPush": "2025-01-02T10:30:00.000Z", "pushCount": 5, "errorCount": 0 },
      "oncourse": { "enabled": true, "lastPush": "2025-01-02T10:31:00.000Z", "pushCount": 142, "errorCount": 0 },
      "results": { "enabled": true, "lastPush": "2025-01-02T10:30:30.000Z", "pushCount": 12, "errorCount": 0 }
    },
    "circuitBreaker": {
      "isOpen": false,
...
| Field | Type | Description |
|-------|------|-------------|
| `state` | string | `idle`, `connecting`, `connected`, `paused`, `error`, `disconnected` |
| `serverUrl` | string\|null | Remote server URL |
| `eventId` | string\|null | Event identifier on remote server |
| `eventStatus` | string\|null | `draft`, `startlist`, `running`, `finished`, `official` |
| `channels` | object | Per-channel push status (xml, oncourse, results) |
| `circuitBreaker` | object | Circuit breaker state (opens after 5 consecutive failures) |
...
| `metadata` | object | Yes | Event creation metadata |
| `metadata.eventId` | string | Yes | Unique event identifier |
| `metadata.mainTitle` | string | Yes | Event title |
| `metadata.subTitle` | string | No | Subtitle |
| `metadata.location` | string | No | Event location |
| `metadata.discipline` | string | No | Discipline code (e.g., `CSL`) |
| `pushXml` | boolean | No | Enable XML push (default: false) |
| `pushOnCourse` | boolean | No | Enable OnCourse push (default: false) |
| `pushResults` | boolean | No | Enable Results push (default: false) |
...
| 400 | `{ "error": "metadata must include eventId and mainTitle" }` |
...
Reconnect to an existing event (no event creation). Used for manual reconnection with known credentials.
...
|-------|------|----------|-------------|
| `serverUrl` | string | Yes | Live-mini server URL |
| `apiKey` | string | Yes | API key from previous event creation |
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `clearConfig` | boolean | No | If true, clears saved connection from settings |
...
| Field | Type | Required | Validation | Description |
|-------|------|----------|------------|-------------|
| `status` | string | Yes | `draft`, `startlist`, `running`, `finished`, `official` | Target event status |
...
| Status | Response |
|--------|----------|
| 400 | `{ "error": "status must be one of: draft, startlist, running, finished, official" }` |
...
The `data` field contains the same structure as the `status` object from `GET /api/live/status`.
