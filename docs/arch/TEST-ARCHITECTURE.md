...
The count is 81 as of the consolidated revision (round 4) (`CONFORMANCE-VECTORS.md` §2). Each of `c123-server` and `live-mini-server` writes
...
- ~~**The exact vector count and coverage for tier 1.**~~ **Resolved.** 35 vectors were written, then 81 after the consolidated revision; the estimate did undercount, as the review suspected. Coverage map and the gaps that remain are in `CONFORMANCE-VECTORS.md` §2–§3. Original text follows for the record.
